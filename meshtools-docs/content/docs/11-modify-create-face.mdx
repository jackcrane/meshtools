...
When the edge selection contains multiple disconnected closed loops, MeshTools creates a separate filled region for each valid loop.
If one selected loop sits inside another, MeshTools treats the inner loop as a hole and leaves that region unfilled.
